...
import { handleTortoiseLog } from './tools/tortoise/tortoise-log.js';
 ...
 */
function wrapHandler(name: string, handler: (args: unknown) => Promise<unknown>) {
  return async (args: Record<string, unknown>) => {
    logger.debug(`Executing tool: ${name}`, args);

    try {
      const result = await handler(args);
      const text = result !== undefined ? JSON.stringify(result, null, 2) : '{"success": true}';
      return {
        content: [
          {
            type: 'text' as const,
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    'svn_cleanup',
    {
      description: 'Recover from an interrupted operation that left the working copy locked',
  ...
  server.registerTool(
    'svn_unlock',
    { description: 'Unlock working copy paths or URLs', inputSchema: UnlockOptionsSchema.shape },
  ...
  server.registerTool(
    'svn_export',
    { description: 'Create an unversioned copy of a tree', inputSchema: ExportOptionsSchema.shape },
  ...
  server.registerTool(
    'svn_changelist_add',
    { description: 'Add files to a changelist', inputSchema: ChangelistAddOptionsSchema.shape },
  ...
  server.registerTool(
    'tortoise_rename',
    { description: '[TortoiseSVN GUI] Rename a file', inputSchema: TortoiseSVNOptionsSchema.shape },
  ...
  server.registerTool(
    'tortoise_unlock',
    { description: '[TortoiseSVN GUI] Unlock files', inputSchema: TortoiseSVNOptionsSchema.shape },
